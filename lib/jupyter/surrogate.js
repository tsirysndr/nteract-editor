...
let js_idx_to_char_idx = (js_idx, text) => {
    let char_idx = js_idx;
...
let char_idx_to_js_idx = (char_idx, text) => {
    let js_idx = char_idx;
    for (let i = 0; i + 1 < text.length && i < js_idx; i++) {
        const char_code = text.charCodeAt(i);
        // check for surrogate pair
...
if ("𝐚".length === 1) {
    ...
    exports.char_idx_to_js_idx = char_idx_to_js_idx = exports.js_idx_to_char_idx = js_idx_to_char_idx = (idx, _text) => {
        return idx;
    };
}
